        ...
        if response.data:
            ...
            with col3:
                ...
                if "Yıl_Ay" in df.columns and "Tutar" in df.columns:
                    aylik_df = df.groupby("Yıl_Ay")["Tutar"].sum().reset_index().sort_values("Yıl_Ay")
                    st.plotly_chart(px.line(aylik_df, x="Yıl_Ay", y="Tutar", markers=True), use_container_width=True)
            with col4:
            ...
            st.dataframe(filtreli_df, use_container_width=True, column_config={"Tutar": st.column_config.NumberColumn("Tutar (TL)", format="%.2f TL")})
            ...
            with col_sil_sec:
                ...
                if not secilen_kayit_detay.empty:
                    # Hata Düzenlendi: .values yerine .iloc[0] kullanılarak temiz veri formatlandı
